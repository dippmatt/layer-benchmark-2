...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
# define NUM_INPUT_BYTES 3072
# define NUM_OUTPUT_BYTES 10
/* USER CODE END PD */
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint8_t *inputAddr  = GLOW_GET_ADDR(mutableWeight, MODEL_serving_default_input_1_0);
	  uint8_t *outputAddr = GLOW_GET_ADDR(mutableWeight, MODEL_StatefulPartitionedCall_0);

	  int k;
	  //for(k=0; k< NUM_INPUT_BYTES; k++)
	  //		  inputAddr[k] = 5;
	  ...
	  PROFILING_EVENT("END");
	  //PROFILING_EVENT("Inf Event 1: Total Model");
	  __enable_irq();
